...
def create_rag_chain(vector_store_path: str = settings.VECTOR_STORE_PATH):
    ...
    def rerank_and_prepare_for_prompt(data: Dict) -> Dict:
        ...
        top_docs_content = [
            result["text"]
            for result in reranked_passages[:settings.RERANKER_TOP_N]
        ]
    ...
    template = """
    You are an expert research assistant. Your goal is to provide a clear and concise answer to the user's question, based ONLY on the following context.
    Do not add any information that is not present in the context. If the answer is not in the context, say "Sorry, I couldn’t find that information in the provided context."
    ...
    answer_chain = (
        RunnableLambda(rerank_and_prepare_for_prompt)
        | prompt
        | llm
        | StrOutputParser()
    )
